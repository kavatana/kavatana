...
Personal portfolio for Kavatana, showcasing projects, capabilities, and
background. Built with Nuxt 3, TypeScript, and minimal CSS tokens.

## How to Deploy

1. Ensure all code passes `npm run typecheck` and `npm run lint`.
2. Commit and push your changes to the `main` branch.
3. GitHub Actions will automatically run the CI pipeline: typecheck, lint, and
   build.
...
2. Open DevTools and verify zero console errors or warnings are thrown.
3. Test the contact form submission on production to ensure the environment
   endpoint is active.
4. Open the Plausible Analytics dashboard and verify live events are firing
   correctly.
...
4. Click the options menu and select **Redeploy** or **Promote to Production**
   to rollback.
...
| `PLAUSIBLE_DOMAIN` | The tracked domain for analytics, for example `kavatana.me`. |
