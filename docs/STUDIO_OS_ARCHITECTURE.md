...
| Private operator    | Owner-controlled private source                      | Bounded workspace evidence and read-only GitHub context        |
...
The verification evidence below reflects the current private operator source.
It is not a claim that every private-operator capability is available in the
public deployment.
...
3. Six private workspace routes return a stable `PRIVATE_OPERATOR_ONLY`
   response before parsing input or invoking operator logic.
...
Evidence snapshot: 2026-07-24, current private operator source.
...
| Next.js public-demo build                                     | 28 static pages generated                                             |
| Build privacy scan                                            | Passed across 722 text build artifacts                                |
| Private workspace API boundary                                | 6 guarded routes returned 403, `PRIVATE_OPERATOR_ONLY`, and `no-store` |
